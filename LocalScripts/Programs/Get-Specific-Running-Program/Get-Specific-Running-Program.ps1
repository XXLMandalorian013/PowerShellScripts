...
function Get-SpecificProgram {
    
    [CmdletBinding()]
    param (
    [Parameter(Mandatory,HelpMessage='Type the programs name ie Chrome')]
    [string]$SingleProgram)

    Try {

        $RunningProgram = Get-Process | Where-object ProcessName -Match $SingleProgram -ErrorAction Stop

        if ($RunningProgram) {
            $RunningProgram | Where-Object { $_.MainWindowTitle }
        }else {
            Write-Host "$SingleProgram is not running"
        }
