...
$TSActions = New-ScheduledTaskAction -Execute "pwsh.exe" -Argument '"\\Server\Folder\Update-Help.ps1"'
...
$TSTriggers = New-ScheduledTaskTrigger -Weekly -WeeksInterval 1 -DaysOfWeek Wednesday -At 12PM
...
Register-ScheduledTask -Action $TSActions -Trigger $TSTriggers -Settings $TSSettings -Principal $TSPrincipal -TaskName "Update PS Help 7.X.X" -Description "Updates PowerShell 7.X.X's Help on a schedule"