...
$UserPrincipal = Read-Host "Type the user email address you wish to block signin for."

$AcctSatus = (Get-AzureADUser -ObjectId $UserPrincipal).AccountEnabled

if ("$AcctSatus" -eq "True") {

    Set-AzureADUser -ObjectID "$UserPrincipal" -AccountEnabled $False

    Write-Host "$UserPrincipal Account Signin blocked."

}

else {

    Write-Host "That users signins are already blocked."
